...
def app(df, first_date, latest_date, customer, id_kh, ne_kh,id_kv,id_tinh):
    ...
    df_selection = df.groupby(['Khu vực', 'date'])['total_order'].sum().reset_index()
    df_selection['date']=df_selection['date'].dt.strftime('%Y/%m/%d')
    df_pivot_KV_number = pd.pivot_table(data=df_selection,index=['Khu vực'], columns='date', values='total_order',aggfunc=sum,fill_value=0)
    df_pivot_KV_number['Tổng'] = df_pivot_KV_number.iloc[:, 0:].sum(axis=1)
    # st.write(df_pivot_KV_number)    
    # Thêm % total_order theo Khu vực
    df_pivot_KV_Ratio= pd.pivot_table(data=df_selection, index='Khu vực',
                                columns='date',
                                values='total_order',
                                aggfunc=sum,
